...
**This post will contain just so many spoilers for the Khan Academy [Cryptography Challenge 101](https://www.khanacademy.org/computing/computer-science/cryptography/cryptochallenge/a/cryptochallenge-introduction). Reading it will ruin most of the fun if you ever plan to try it on your own.**
...
3. Binary string is XOR'd against a <span class="term" data-def="A shared secret string used to scramble a message. The 'one-time' part is the whole point: reuse it and the encryption becomes breakable.">one-time pad</span>, which in this case is a string of letters converted to ones and zeroes where vowels are ones and consonants are zeroes.
